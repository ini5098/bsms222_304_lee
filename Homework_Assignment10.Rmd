...
![(Image courtesy of Karl Broman)](https://rafalab.github.io/dsbook/dataviz/img/fig8b.png)
...
![(Images courtesy of Karl Broman)](https://rafalab.github.io/dsbook/dataviz/img/fig2d.png)
...
```{r}
mutate(dat, state = reorder(state, rate)) 
dat %>% select(state, rate)
```
...
murders %>% mutate(rate = total/population*100000) %>% 
  group_by(region) %>%
  summarize(avg = mean(rate)) %>%
  mutate(region = factor(region), region = reorder(region, avg)) %>%
  ggplot(aes(region, avg)) +
  geom_bar(stat="identity") +
  ylab("Murder Rate Average")
```
...
dat <- us_contagious_diseases %>%  
  ...
  mutate(rate = count / population * 10000 * 52 / weeks_reporting, state = reorder(state, rate))
dat %>% ggplot(aes(state, year, fill = rate)) +
  geom_raster() + 
...
dat <- us_contagious_diseases %>%  
  filter(weeks_reporting >= 10 & disease=="Smallpox" & !is.na(population)) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting, state = reorder(state, rate))
dat %>% ggplot(aes(year, rate, col = state, label = state)) +
  geom_line()
```
...
dat <- us_contagious_diseases %>%  
  ...
  mutate(rate = count / population * 10000 * 52 / weeks_reporting, disease = reorder(disease, rate))
dat %>% ggplot(aes(year, rate, col = disease, label = disease)) +
  geom_line()
```
...
dat <- us_contagious_diseases %>%  
  ...
  summarise(rate = sum(count) / sum(population) * 10000 * 52 / weeks_reporting) %>% 
  mutate(disease = reorder(disease, rate))
dat %>% ggplot(aes(year, rate, col = disease, label = disease)) +
  geom_line()
```
