...
```{r}
data("murders")
my_states <- murders %>% mutate(murders, rate =  total / population * 100000, rank = rank(-rate)) %>% filter(region %in% c("Northeast", "West") & rate < 1) %>% select(state, rate, rank)
my_states
```
...
```{r}
ref_avg <- NHANES %>% filter(AgeDecade == " 20-29", Gender == "female") %>% summarise(average = mean(BPSysAve, na.rm = TRUE)) %>% pull()
ref_avg
```
3\. Now report the min and max values for the same group.
```{r}
NHANES %>% filter(AgeDecade == " 20-29", Gender == "female") %>% summarise(minimum = min(BPSysAve, na.rm = TRUE), maximum = max(BPSysAve, na.rm = TRUE))
```
...
```{r}
NHANES %>% filter(AgeDecade == " 40-49", Gender == "male") %>% group_by(Race1) %>% summarise(average = mean(BPSysAve, na.rm = TRUE)) %>% arrange(average)
```
...
compute_s_n <- function(m){
  x <- 1:m
  y <- 1:sum(x)
  tibble(n = m, s_n = sum(x), s_n_2 = sum(y))
}
map_df(m, compute_s_n)
```